...
int concatenaLista(ListaCont *listaA, ListaCont listaB){
    ...
    int i;
    int tamanhoListaA = tamanhoLista(*listaA);
    int tamanhoListaB = tamanhoLista(listaB);
    int tamanho = tamanhoListaA + tamanhoListaB; //calcula o tamanho das listas

    if (tamanho < TAM_MAX_LISTA){ // ve se possui espaço para a ooncatenacao
        for (i=listaB.ini; i< tamanhoListaB; i++){
             insereLista(listaA, listaB.nodos[i],tamanhoListaA); // insere se possuir espaco
             tamanhoListaA ++; //intera para a proxima operacao
        }
        return 1
    } else {
        return 0 // retorna 0 caso nao de para concatenar
    }
}
...
int insereUnicoLista(ListaCont *lista, NodoLCont nodo, int pos){
    ...
    int i;
    for (i=lista->ini; i <  lista->fim; i++){
        if (lista->nodos[i].matricula == nodo.matricula){ // verifica se a matricula eh igual a outra
            return 0;
        }
    }

    return insereLista(lista, nodo, pos); // retorna 1
